...
export async function POST(request: NextRequest) {
  try {
    ...
    if (event === 'tray_change') {
      ...
      // Only when the tray reports a real serial AND the brand actually carries
      // one: Creality's CFS reports a shared material-type code rather than a
      // per-spool serial, so matching on it would assign whichever spool last
      // carried that code.
      const canMatchBySerial = isValidTrayUuid(tray_uuid) && await traySupportsSerial(tray_entity_id);

      // Populated only when a serial match is actually attempted, so a tray that
      // reports no usable serial doesn't pay for an extra full Spoolman scan.
      let diag: Awaited<ReturnType<typeof client.getMatchDiagnostics>> = {
        searchedFor: tray_uuid ?? '',
        tagEntries: [],
        nfcUidEntries: [],
        spoolsScanned: 0,
      };
      ...
      if (canMatchBySerial) {
        ...
        diag = await client.getMatchDiagnostics(tray_uuid);
      ...
      // the lookups above are by RFID serial (extra.tag) and pre-populated NFC
      // UID, neither of which every spool has. A spool that is correctly
